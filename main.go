...
func main() {
	...
	items := make([]itemT, N)
	itemsM := make(map[keyT]bool)
	for i := 0; i < N; i++ {
		for {
			//key := rand.Int63n(10000000000000000)
			newItem := itemT{}
			newItem.key = keyT(RandStringBytesMaskImprSrcUnsafe(keyLength))
			newItem.val = valT(RandStringBytesMaskImprSrcUnsafe(valueLength))
			if !itemsM[newItem.key] {
				itemsM[newItem.key] = true
				items[i] = newItem
				//if len(items[i].key) != 16 {
				//	panic("!")
				//}
